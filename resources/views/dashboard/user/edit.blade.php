...
@section('content')
    <x-ui.card>
        <div class="mb-4">
            <h3 class="mb-6">Edit User</h3>
        </div>

        <form action="{{ route('dashboard.users.update', ['user' => $user->id]) }}" method="post" autocomplete="off"
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <x-ui.form-group>
                    ...
                    <x-ui.input-element type="text" name="first_name" id="first_name" placeholder="Masukan nama depan"
                        :validate="$errors->has('first_name')" value="{{ $user->first_name }}" />
                    @error('first_name')
                ...
                <x-ui.form-group>
                    ...
                    <x-ui.input-element type="text" name="last_name" id="last_name" placeholder="Masukan nama belakang"
                        :validate="$errors->has('last_name')" value="{{ $user->last_name }}" />
                    @error('last_name')
                        <span class="block mt-1 text-danger-500">{{ $message }}</span>
                    @enderror
                </x-ui.form-group>
